...
handler handle (Authenticated claim) =
  Handler $ withExceptT toServantError $ Namespace <$> current handle claim
handler _ _ =
...
current :: Handle -> Claim -> ExceptT Error IO Account
current Handle {connectionPool, jwtSettings} (Claim id) =
